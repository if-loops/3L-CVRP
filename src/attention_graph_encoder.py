...
class MultiHeadAttentionLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, input_dim, num_heads, feed_forward_hidden=512, **kwargs):
        ...
        self.ff1 = tf.keras.layers.Dense(
            feed_forward_hidden, name="ff1", kernel_initializer="he_uniform"
        )
        self.ff2 = tf.keras.layers.Dense(
            input_dim, name="ff2", kernel_initializer="he_uniform"
        )
        # self.keras_norm = tf.keras.layers.LayerNormalization(
        #    axis=-1, center=True, scale=True, trainable=True, name="input_normalized",
        # )
